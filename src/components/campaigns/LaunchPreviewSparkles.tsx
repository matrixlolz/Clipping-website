...
/**
 * Ported from UIUX/folder glass ui — Sparkles overlay (DefaultProject generating/live).
 */
interface LaunchPreviewSparklesProps {
  count?: number;
  fading?: boolean;
...
export function LaunchPreviewSparkles({
  count = 16,
  fading = false,
  variant = "live",
}: LaunchPreviewSparklesProps) {
  ...
  return (
    ...
    >
      ...
      {[...Array(count)].map((_, idx) => {
        ...
        const opacity = variant === "generating" ? 0.15 + (idx % 4) * 0.1 : 0.2 + (idx % 3) * 0.12;
        return (
          <div
            key={idx}
            ...
            style={{
              ...
              animationDuration: `${(variant === "generating" ? 2 : 1.8) + (idx % 3) * (variant === "generating" ? 0.5 : 0.4)}s`,
            }}
          >
            <svg
